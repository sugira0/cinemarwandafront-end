...
// In production on Vercel: use relative /api paths (Vercel rewrites proxy to backend)
// In development: use localhost:5000 directly
// If VITE_API_ORIGIN is explicitly set: use that value
export const API_ORIGIN = import.meta.env.DEV
  ? (configuredOrigin || 'http://localhost:5000')
  : (configuredOrigin || '');  // empty = use relative paths via Vercel proxy

export const API_BASE_URL = API_ORIGIN ? `${API_ORIGIN}/api` : '/api';
const UPLOADS_BASE_URL = import.meta.env.DEV
  ? `${API_ORIGIN}/uploads`

...
export default function Register() {
  ...
  const handleRequestOtp = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to send sign-up code');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to resend sign-up code');
    } finally {
      setResending(false);
    }
  };
    ...
    <div className="auth-page">
      <form className="auth-form" onSubmit={step === 'details' ? handleRequestOtp : handleVerifyOtp}>
        ...
        <p className="auth-note">
          {step === 'details'
            ? 'Email verification is now required for every new account. You can still add your Rwanda mobile number during sign-up.'
